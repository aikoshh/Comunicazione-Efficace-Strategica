...
/** Normalizza il payload per evitare undefined in render */
const normalizeResult = (r?: Partial<AnalysisResult>): AnalysisResult => ({
  score: typeof r?.score === 'number' ? r!.score : 0,
  strengths: Array.isArray(r?.strengths) ? (r!.strengths as string[]) : [],
  areasForImprovement: Array.isArray(r?.areasForImprovement)
    ? (r!.areasForImprovement as Array<{ suggestion?: string; example?: string }>)
    : [],
  suggestedResponse:
    r?.suggestedResponse && typeof r.suggestedResponse === 'object'
      ? {
          short: (r.suggestedResponse as any).short ?? '',
          long: (r.suggestedResponse as any).long ?? '',
        }
      : { short: '', long: '' },
});
...
const ScoreCircle: React.FC<{ score: number }> = ({ score }) => {
  ...
  return (
    <div style={styles.scoreContainer}>
      ...
      <div style={{ ...styles.scoreText, color: strokeColor, animation: 'popIn 0.5s 0.8s ease-out both' }}>
...
export const AnalysisReportScreen: React.FC<AnalysisReportScreenProps> = ({ result, exercise, onRetry, onNext }) => {
  ...
  const hoverStyle = `
    ...
    .primary-button:active, .secondary-button:active { transform: translateY(0); filter: brightness(0.95); }
  `;
          ...
          <div style={{ ...styles.feedbackCard, animation: 'fadeInUp 0.5s 0.2s ease-out both' }}>
            ...
            <ul style={styles.list}>
              {(safe.strengths ?? []).length > 0 ? (
                ...
                <li style={styles.listItem}>Nessun punto di forza disponibile.</li>
              )}
            </ul>
          </div>
          ...
          <div style={{ ...styles.feedbackCard, animation: 'fadeInUp 0.5s 0.4s ease-out both' }}>
            ...
            <ul style={styles.list}>
              {(safe.areasForImprovement ?? []).length > 0 ? (
                (safe.areasForImprovement ?? []).map((item, index) => (
                  <li key={index} style={styles.listItem}>
                ...
                <li style={styles.listItem}>Nessuna area di miglioramento.</li>
              )}
            </ul>
          </div>
        </div>
